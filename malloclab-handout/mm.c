...
int statusCountMalloc = 0;
int statusCountFree = 0;
...
static BlockInfo* searchFreeList(size_t reqSize) {
  ...
  while(freeBlock != NULL){
    // check to see if this block is large enough to fit reqSize
    if(SIZE(freeBlock -> sizeAndTags) >= reqSize){
      // if it is, then return a pointer to this block
      return freeBlock;
    }
    else{
      // if not, then move onto the next block
      freeBlock = freeBlock -> next;
    }
  }

  // if we make it to the end of the list without finding a block, then return NULL
  return NULL;
}
...
void* mm_malloc(size_t size) {
  ...
  if (size == 0){     // Zero-size requests get NULL.
    return NULL;
  }

  size += WORD_SIZE;                    // Add one word for the initial size header.
  if (size <= MIN_BLOCK_SIZE) {         // Note that we don't need to boundary tag when the block is used!
    // Make sure we allocate enough space for a blockInfo in case we free this block (when we free this block, we'll need to use the next pointer, the prev pointer, and the boundary tag).
    reqSize = MIN_BLOCK_SIZE;
  } else {
    reqSize = ALIGNMENT * ((size + ALIGNMENT - 1) / ALIGNMENT);     // Round up for correct alignment
  }

  ptrFreeBlock = searchFreeList(reqSize);           // find a free block that is big enough for reqSize
  if(ptrFreeBlock == NULL){                         // couldnt find a free block large enough
    requestMoreSpace(reqSize);                      // ask the kernal for more space in the heap
    ptrFreeBlock = searchFreeList(reqSize);         // search again, this time it will be successful since there is more space in the heap
  }

  removeFreeBlock(ptrFreeBlock);                    // remove this block from the free list since we are going to use or edit it
  blockSize = SIZE(ptrFreeBlock -> sizeAndTags);    // this will get us the size of the returned block (but we may not need all of it)
  precedingBlockUseTag = (ptrFreeBlock -> sizeAndTags) & TAG_PRECEDING_USED;    // mask to tell if prec. block is used

  if((blockSize - reqSize) >= MIN_BLOCK_SIZE){      // if the returned block is larger than we need by as least the minimum block size then we split
    newFreeBlock = (BlockInfo* )UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);    // place a blockinfo for the new free block
    newFreeBlock -> sizeAndTags = ((size_t)(blockSize - reqSize) & -2) | TAG_PRECEDING_USED;    // set size and tags of free block approperately
    //set the boundary tag at the end of the free block
    *((size_t *)UNSCALED_POINTER_SUB((BlockInfo *)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize), WORD_SIZE)) = newFreeBlock -> sizeAndTags;
    insertFreeBlock(newFreeBlock);                  // put this new free block back into the free list

    ptrFreeBlock -> sizeAndTags = reqSize | TAG_USED | precedingBlockUseTag;    // set size and tags of used block approperately
  }
  else{   // this is the case that the returned block is the smallest size that we need
      ptrFreeBlock -> sizeAndTags = (ptrFreeBlock -> sizeAndTags) | TAG_USED;   // set used tag accordingly
      ((BlockInfo *)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize)) -> sizeAndTags = (((BlockInfo *)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize)) -> sizeAndTags) | TAG_PRECEDING_USED; // set the preceding used tag of the next block accordingly
  }
...
void mm_free(void *ptr) {
  ...
  blockInfo = (BlockInfo *)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);      // get the block info from payload pointer
  blockInfo -> sizeAndTags = (blockInfo -> sizeAndTags) & -2;         // set used tag to 0
  followingBlock = (BlockInfo *)UNSCALED_POINTER_ADD(blockInfo, SIZE(blockInfo -> sizeAndTags));  // get blockInfo for follwing block
  *((size_t *)UNSCALED_POINTER_SUB(followingBlock, WORD_SIZE)) = blockInfo -> sizeAndTags;      // set the boundary tag accordingly

  if(followingBlock < (BlockInfo *)mem_heap_hi()){
    followingBlock -> sizeAndTags = followingBlock -> sizeAndTags & -3;     // change prev. used tag of next block accordingly
  }
