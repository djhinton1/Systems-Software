...
{//TODO implement this function
	char *argv[MAXARGS];
	int whichGround = parseline(cmdline, argv);
	...
	// if this is the child process
		...
		if(execvp(argv[0], argv) < 0){
			printf("Could not execute command... Sorry.");
		}
		exit(0);
	}
	...
	// if this is the parent process
		...
		// if the child process is in the foreground, then we wait.
		if(!whichGround){
			addjob(jobs, pid, FG, cmdline);
			sigprocmask(SIG_UNBLOCK, &mask, NULL);
			waitfg(pid);
		}
		// otherwise child process is in the background.
		else{
...
void do_bgfg(char **argv)
{
	...
	// and it needs to be told to continue running (unsing kill)
	// we do want to kill with the sig_int handler (control-c)
	int i;
	int noCommands = 2;
	int argToSwitch = 0;
	char *commands[noCommands];
